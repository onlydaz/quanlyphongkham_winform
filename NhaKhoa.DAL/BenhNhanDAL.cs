    ...
    public class BenhNhanDAL
    {
        ...
        public List<BenhNhan> Search(string ma = "", string ten = "", string sdt = "")
        {
            using (var ctx = new NhaKhoaContext())
            {
                ...
                // N?u c� keyword, t�m ki?m theo OR logic (m� HO?C t�n HO?C SDT ch?a keyword)
        ...
        public List<BenhNhan> GetByTrangThai(string trangThai)
        {
            using (var ctx = new NhaKhoaContext())
            {
                ...
                try
                {
                    ...
                    {
                        // N?u trangThai l� null ho?c empty, l?y c�c record c� TrangThai l� null ho?c empty
                        query = query.Where(x => x.TrangThai == null || x.TrangThai == "" || x.TrangThai == trangThai);
                    }
                    else
                    {
                        // So s�nh ch�nh x�c, kh�ng ph�n bi?t hoa thu?ng
                    ...
                    if (result.Count > 0)
                    {
                        System.Diagnostics.Debug.WriteLine($"V� d? TrangThai: '{result.First().TrangThai ?? "NULL"}'");
                    }
                ...
                {
                    // N?u c� l?i, th? load t?t c? r?i filter trong memory
                    System.Diagnostics.Debug.WriteLine($"L?i query TrangThai: {ex.Message}");
                    var all = ctx.BenhNhans.ToList();
                    System.Diagnostics.Debug.WriteLine($"T?ng s? b?nh nh�n: {all.Count}");
