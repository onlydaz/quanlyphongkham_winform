    ...
    public partial class FRM_Dichvu : Form
    {
        ...
        private void btnThemCD_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Theo ERD mới: CHANDOAN chỉ có MaCD và TenChuanDoan (không có MaLS trong database)
        ...
        private void btnThemDT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Theo ERD mới: DIEUTRI có MaDT, MaCD (FK -> CHANDOAN), TenDieuTri, DonViTinh, DonGia
                // MaCD có thể lấy từ chẩn đoán đã chọn trong grid hoặc để null
                // MaLS không còn trong bảng DIEUTRI, liên kết được quản lý ở bảng LAMSAN
                string maCD = null;
                // Lấy MaCD từ chẩn đoán đã chọn (nếu có)
                ...
                // Hoặc lấy từ textbox nếu đã chọn chẩn đoán
                ...
                var dieuTri = new DieuTri
                {
                    ...
                    DonGia = donGia,
                    MaCD = string.IsNullOrWhiteSpace(maCD) ? null : maCD, // FK đến CHANDOAN, có thể null
                    MaLS = null // Theo ERD mới, DIEUTRI không có MaLS
                };
        ...
        private void dgvDieuTri_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            // MaCD có thể hiển thị trong grid nhưng không có control để chỉnh sửa
        }
        ...
        private void mnuCapNhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (!string.IsNullOrWhiteSpace(txtmaDT.Text) && !string.IsNullOrWhiteSpace(txttenDT.Text))
                {
                    ...
                    // Lấy MaCD từ grid hoặc giữ nguyên từ bản ghi hiện tại
                    string maCD = null;
